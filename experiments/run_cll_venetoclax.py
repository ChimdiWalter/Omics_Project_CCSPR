...
def main() -> None:
    ...
    ap.add_argument("--config", default="configs/cll_venetoclax.yaml")
    args = ap.parse_args()
    ...
    metrics.to_csv(global_tables_dir / "cll_venetoclax_metrics.csv", index=False)
    ...
    _save_table_both(
        ...
        global_tables_dir / "cll_venetoclax_main_results.csv",
    )
    _save_table_both(
        main_summary,
        tables_dir / "main_results.csv",
        global_tables_dir / "cll_venetoclax_main_results_table.csv",
    )
    ...
    _save_table_both(
        ...
        global_tables_dir / "cll_venetoclax_ablation_summary.csv",
    )
    _save_table_both(
        ablation_summary,
        tables_dir / "ablation_summary.csv",
        global_tables_dir / "cll_venetoclax_ablation_summary_table.csv",
    )
    ...
    plot_tip_eu_vs_ricci(aux["tip_eu"], aux["tip_ricci"], str(global_fig_dir / "cll_venetoclax_tip_eu_vs_ricci.png"))
    plot_lifetime_prominence(
        aux["lifetime_eu"],
        aux["lifetime_ricci"],
        aux["prominence_eu"],
        aux["prominence_ricci"],
        str(global_fig_dir / "cll_venetoclax_h1_lifetime_eu_vs_ricci.png"),
        str(global_fig_dir / "cll_venetoclax_h1_prominence_eu_vs_ricci.png"),
    )
    plot_f1_bars_with_ci(metrics_main, str(global_fig_dir / "cll_venetoclax_f1_bars_ci.png"))
    if not ablation.empty:
        plot_lambda_robustness(ablation, str(global_fig_dir / "cll_venetoclax_lambda_robustness.png"))
