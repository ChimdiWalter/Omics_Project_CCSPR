...
\large Resource-Bounded Execution, Supporting Results, and Reproducibility}
\author{}
\date{}
...
\item \textbf{GSE161711 results}: \texttt{results/metrics\_cll\_venetoclax\_gse161711.csv}, \texttt{results/main\_results\_cll\_venetoclax\_gse161711.csv}, \texttt{results/ablation\_summary\_cll\_venetoclax\_gse161711.csv}
\item \textbf{Single-cell substitute (GSE165087)}: \texttt{data/cll\_rs\_scrna/data.h5ad}
\item \textbf{GSE165087 results}: \texttt{results/metrics\_cll\_rs\_scrna\_gse165087.csv}, \texttt{results/main\_results\_cll\_rs\_scrna\_gse165087.csv}
...
\item \texttt{representative\_diagnostics\_summary.csv} --- TIP and sparsity diagnostics
\item \texttt{geometry\_runtime\_cost\_summary.csv} --- Runtime and resource usage by job
\item \texttt{ablation\_profiles\_combined.csv} --- Combined ablation profiles
...
12849767 & manuscript refresh & COMPLETED & --- & --- & --- \\
...
The key observation is that the manuscript-safe configuration (\texttt{max\_cells=320}, \texttt{n\_pcs=20}, 1 split, 1 bootstrap) achieved a ${\sim}34\times$ reduction in peak RSS compared to the lowest-memory failed run.
This disproportionate reduction was partly enabled by a code-level fix (Section~\ref{sec:codefix}).
...
\item 1 harmonic bootstrap
\item 1 CC-SPR bootstrap
...
A strictly bounded ablation path was also prepared:
\begin{itemize}[leftmargin=1.5em]
...
\begin{verbatim}
comparison,n_pairs,mean_delta,std_delta,median_delta,paired_t_stat,
  paired_t_p,wilcoxon_stat,wilcoxon_p
ccspr_vs_harmonic,18,0.0272,0.1733,0.0335,0.6670,0.5137,69.0,0.4951
ccspr_vs_standard,18,-0.1861,0.1726,-0.1398,-4.5744,0.000269,10.0,0.000328
ricci_sparse_vs_euclidean_sparse,18,0.0460,0.1857,0.0538,1.0518,0.3076,
  59.0,0.2645
\end{verbatim}
...
\end{verbatim}
BRCA is used only as supplementary evidence confirming the pattern seen on LUAD: standard baseline dominance, with topology-derived methods providing complementary diagnostic value.

\subsection{GSE161711 bulk validation (main results)}
...
CC-SPR exceeds harmonic by $+0.018$ in mean F1, with substantially higher variance due to the sensitivity of sparse representatives to individual splits.
...
This identical performance is expected for a small bounded split where the dominant signal is the class structure itself.
...
Lambda sensitivity on the ablation-safe configuration:
\begin{itemize}[leftmargin=1.5em]
\item $\lambda = 10^{-6}$: F1 $= 0.847$
\item $\lambda = 10^{-5}$: F1 $= 0.850$
\end{itemize}

The ablation-safe configuration provides more informative results than the manuscript-safe tie, showing CC-SPR exceeding harmonic while trailing standard---consistent with the pattern observed on bulk datasets.
Peak RSS for the ablation-safe run was ${\sim}$9.3\,GB, well within the 32\,GB allocation.

% ============================================================
\section{Ablation Details}
% ============================================================

\subsection{Full ablation sweep summary}
...
All ablation results are cached in \texttt{results/metrics\_all\_datasets.csv} and summarized in \texttt{results/tables/ablation\_profiles\_combined.csv}.

\subsection{Cross-dataset ablation divergence}

The most informative finding from the ablation analysis is that LUAD and GSE161711 prefer different settings:
\begin{itemize}[leftmargin=1.5em]
...
This divergence supports the central claim: geometry and sparsity parameters are genuine modeling decisions whose optimal values depend on dataset structure.
...
\begin{tabular}{llll}
\toprule
Backend & Implementation status & Benchmark status & Paper treatment \\
\midrule
...
\label{tab:backendstatus}
\end{table}

\subsection{What was intentionally deferred}

The following items were intentionally moved out of the critical path:
\begin{enumerate}[leftmargin=1.5em]
...
% ============================================================
\section{Geometry Diagnostics Summary Tables}
% ============================================================

The following lightweight tables were derived from cached outputs without rerunning topology loops.
...
\bottomrule
\end{tabular}
\caption{Geometry backend comparison across datasets.
CC-SPR exceeds harmonic on GSE161711 but not on LUAD.
...
% ============================================================
\section{Interpretation of Cross-Dataset Patterns}
% ============================================================

The cross-dataset results reveal a consistent pattern that informs how CC-SPR should be deployed in practice.

\paragraph{Why standard baselines dominate on prediction.}
Standard classifiers on high-variance features exploit the strongest axis of variation in the data, which in TCGA and similar well-curated cohorts typically aligns with the class label.
Topology-derived features capture orthogonal structural information---cycles, holes, and connectivity patterns---that may not correlate with the supervised signal even when they reflect genuine biological structure.
This is analogous to how unsupervised clustering and supervised classification can disagree: both are valid but measure different properties of the data.

\paragraph{Why CC-SPR modestly exceeds harmonic PH.}
On both GSE161711 and the ablation-safe GSE165087 configuration, CC-SPR achieves a small advantage over harmonic PH ($+0.018$ and $+0.025$ respectively).
The likely mechanism is that sparse representatives project to a more compact gene set, reducing the dilution effect of including many low-signal genes in the feature vector.
This is consistent with the elastic-net interpretation: the $\ell_1$ penalty acts as a built-in feature selector within the topological pipeline.

\paragraph{Why LUAD and GSE161711 prefer different geometry backends.}
LUAD ($n \approx 500$, high-dimensional TCGA expression) benefits from Ricci curvature correction, likely because the Euclidean neighborhood graph contains shortcut edges between subtypes that create spurious topological features.
GSE161711 ($n = 57$, lower-dimensional bulk RNA-seq) does not benefit, likely because the smaller sample size means that Ricci flow's reweighting is dominated by sampling noise rather than genuine geometric correction.
...
\subsection{Key commands (for reference)}
\begin{verbatim}
# LUAD anchor
python3.11 experiments/run_tcga_luad.py --config configs/tcga_luad.yaml
