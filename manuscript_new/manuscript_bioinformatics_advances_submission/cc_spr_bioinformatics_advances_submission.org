...
\citep{carlsson2009, edelsbrunner2010}, which has been applied to gene
expression landscapes \citep{rizvi2017}, protein structure
\citep{xia2014}, tumor microenvironments \citep{vipond2021}, single-cell
trajectories \citep{moon2019phate}, neural population coding
\citep{giusti2015}, and viral evolution \citep{chan2013topology}.
...
Harmonic persistent homology \citep{harmonicPH} addresses this gap by
...
- *Arabidopsis root atlas* (GSE123013, \citealt{denyer2019}, scRNA-seq,
...
barcode $\{[b_i, d_i)\}$ \citep{zomorodian2005, edelsbrunner2010}. We
...
feature space \citep{harmonicPH, friedman1998}.
...
\citep{zou2005elasticnet}: $\lambda_1$ plays the LASSO role promoting
...
$\varepsilon_\infty$ \citep{chazal2014persistence}. We implement five
backends:
...
The Ollivier--Ricci curvature \citep{ollivier2007,ollivier2009} of an
edge $(u,v)$ is $\kappa(u,v) = 1 - W_1(\mu_u, \mu_v)/d(u,v)$, where
$W_1$ is the Wasserstein-1 distance between neighborhood measures.
Discrete Ricci flow \citep{ni2019community} iteratively updates edge
weights:
...
Diffusion distances \citep{coifman2006diffusion} are derived from the
...
PHATE \citep{moon2019phate} transforms diffusion operators via a log
...
DTM \citep{chazal2011dtm, chazal2017robust} replaces pairwise distances
...
\citealt{gudhi2014}), and sparse optimization ($O(|E|^2 \cdot |F|)$ per
...
CC-SPR: the original Ricci-only result (F1 $= 0.3133$) improves to DTM
$= 0.5315$ at pilot scale and Ricci $= 0.6339$ at full scale.
...
PHATE-like produces zero Gini, suggesting over-smoothing.
...
not translate to the highest classification F1. On GSE161711, Ricci
creates the most persistent topological features ($H_1$ lifetime
$= 0.1248$) but trails Euclidean on classification, illustrating that
persistent topology captures structure complementary to class
...
*Takeaway:* Topological diagnostics vary systematically across backends,
providing information unavailable from classification metrics alone.
...
Over-representation analysis (ORA) using gseapy \citep{gseapy} against
...
(Table [[#tab:geneoverlap][[tab:geneoverlap]]]), directly supporting the
thesis that geometry choice controls which biological signal is
extracted.
...
marker---not detected at pilot scale. Ricci recovers innate immunity
(DEFB4A) and squamous differentiation (SPRR3) markers. Diffusion and
PHATE-like both highlight cancer-testis antigens (MAGEA family).
...
(rank 7, Euclidean) is a canonical LUAD lineage marker.
...
Following \citet{harmonicPH}, we construct sample-level cycle-weight
heatmaps for the top 15 $H_1$ bars (120 balanced LUAD samples, Euclidean
backend). CC-SPR produces only 18% nonzero entries (vs 50% for harmonic,
2.8$\times$ denser). Ward hierarchical clustering recovers three sample
clusters with comparable purity (0.43 for CC-SPR vs 0.47 for harmonic),
demonstrating that sparsification preserves clustering structure.

#+CAPTION: Sample-level cycle-weight heatmaps for LUAD (120 samples
$\times$ 15 $H_1$ bars). *Top*: CC-SPR---18% nonzero. *Bottom*:
Harmonic---50% nonzero.
[[file:figures/gurnari_style_comparison.png]]

*Takeaway:* CC-SPR achieves 2.8$\times$ sparser representations while
...
The sparsity parameter $\lambda$ shows contrasting behavior across
datasets (Tables [[#tab:lambda_luad][[tab:lambda_luad]]] and
[[#tab:lambda_gse][[tab:lambda_gse]]]). On LUAD, $\lambda$ has no
effect; on GSE161711, most backends lose F1 at higher $\lambda$, but DTM
uniquely /improves/, suggesting its density-smoothed distances produce
representatives where additional sparsity removes noise rather than
signal.
...
| dtm          | 0.588                    | 0.628                    |
#+CAPTION: GSE161711 lambda ablation: DTM uniquely improves with higher
$\lambda$.
...
(Table [[#tab:ablationbest][[tab:ablationbest]]]) confirms cross-dataset
divergence: LUAD benefits from Ricci geometry and compact top-$K$;
GSE161711 benefits from Euclidean geometry and broad top-$K$.
...
#+CAPTION: Best ablation settings. Cross-dataset divergence supports
backend-sensitive behavior.

#+CAPTION: Cross-dataset ablation profiles showing dataset-specific
sensitivity to every hyperparameter axis.
...
Three larger scRNA configurations failed under cluster limits
(Table [[#tab:scRNAresource][[tab:scRNAresource]]]); the bounded
configuration ($n = 320$, 1 split, 1 bootstrap) completed with all five
backends producing identical F1 $= 0.9254$, establishing pipeline
feasibility but not a meaningful backend comparison.
...
DTM outperforms Euclidean on Arabidopsis root scRNA-seq (F1 $= 0.67$ vs
$0.59$; 500 cells, 16 Leiden clusters, 3-fold CV), consistent with its
LUAD advantage. Euclidean selects transcription factors (FLA12, AGL1,
IAA29), while DTM emphasizes cell wall remodeling (GATA2, CSLA9, EXPB3).
Three genes (IAA29, AT5G44005, PAR2) appear in both backends, suggesting
auxin signaling is geometry-robust.
...
*Takeaway:* DTM's advantage generalizes across biological domains with
coherent gene selections.
...
dedicated stability and permutation study.
...
On 5,000 variable genes, all five backends completed $\geq 986/1000$
iterations ($> 98.6\%$). The rank order of support sizes and Gini values
is preserved between $n_{\text{boot}} = 5$ and $n_{\text{boot}} = 1000$
...
variable genes. Concentration ranking is preserved from the
$n_{\text{boot}} = 5$ diagnostics.
...
workstation); (ii) at $n = 1{,}000$ cells (memory $\sim 14$ GB),
...
$n_{\text{boot}} = 10$, both Euclidean and DTM completed 10/10
iterations in $\sim 6$ h total (euclidean: support 46, Gini 0.323; dtm:
support 48, Gini 0.289). This is a fundamental limitation of the Rips +
elastic-net approach, not an implementation artifact, and motivates
future work on approximate persistent homology (sparse Rips, witness
complexes) and better-conditioned cycle optimizers for large single-cell
atlases.
...
analysis. The 2.8$\times$ sparsification preserves clustering structure
while dramatically improving interpretability. Second, the backend-aware
framework reveals that geometry choice materially controls which
biological signal is extracted---zero genes are shared across all six
configurations on LUAD. Third, TIP profiles provide a principled
stability diagnostic, enabling practitioners to assess which features
are robustly associated with topological structure.

Even when CC-SPR trails the standard baseline on F1, the sparse
representatives and TIP profiles identify gene--topology associations
and geometry sensitivities that standard classifiers cannot detect.
...
with ambient-space proximity (GSE161711). *Ricci* helps when curvature
correction suppresses misleading shortcut edges between subtypes
(full-scale LUAD); its advantage strengthens with sample size. *DTM*
excels on heterogeneous data with density irregularity (pilot LUAD) and
produces the most compact support (GSE161711, 24 features). *Diffusion*
provides intermediate denoising via spectral smoothing. *PHATE-like*
ranks last on both LUAD and GSE161711; its zero Gini on LUAD suggests
that the lightweight potential-distance surrogate over-smooths
topological signal.
...
LUAD subtypes \citep{rizvi2017}, particularly the proximal-inflammatory
subtype.

The zero-overlap finding is evidence that different geometric
perspectives highlight different biological programs. Practitioners can
compare TIP profiles to identify geometry-robust genes (core signal)
versus geometry-specific genes.

Cross-domain consistency strengthens these findings: DTM outperforms
Euclidean on both LUAD and Arabidopsis, with biologically coherent gene
selections in both domains.
...
\citep{rizvi2017, xia2014, vipond2021, giusti2015, chan2013topology}.
Harmonic PH \citep{harmonicPH} introduced the bridge from persistent
...
Ollivier--Ricci curvature
\citep{ollivier2007, ollivier2009, ni2019community} has been used for
community detection, but its connection to persistent homology through
filtration perturbation appears novel. Diffusion maps
\citep{coifman2006diffusion} and PHATE \citep{moon2019phate} are widely
used for dimensionality reduction; using these as persistent-homology
backends within a sparse representative framework has not been
systematized. DTM-based topology \citep{chazal2011dtm, chazal2017robust}
provides robust persistent homology, but its integration into a
representative-cycle framework with feature projection appears
unexplored. Sparse representations in TDA have been studied through
$\ell_1$ optimal cycles \citep{chen2011hardness, dey2011optimal}, but in
computational-geometry settings without the biological
...
2. *Backend ranking is scale-sensitive.* Pilot LUAD ranking (DTM $>$
   Ricci) reverses at full scale.

3. *Evaluation protocol asymmetry.* The standard baseline selects
   variable genes from the full dataset at full LUAD scale.

4. *PHATE-like uses a lightweight surrogate.* Results may differ with
   the complete PHATE pipeline \citep{moon2019phate}.

5. *Single-cell evidence is resource-bounded.*

6. *Arabidopsis is a targeted two-backend test.*

7. *Enrichment is exploratory.*
...
convergence; the full PHATE pipeline; memory-optimized single-cell
implementations; and extension to higher homology dimensions.
...
2. *Geometry controls which biology is extracted.* Zero genes are shared
   across all six configurations on LUAD. DTM uniquely recovers
   xenobiotic metabolism (adj. $p = 7 \times 10^{-4}$).

3. *Sparse representatives concentrate interpretation.* CC-SPR produces
   2.8$\times$ sparser sample-bar matrices while preserving comparable
...
(GSE123013) from GEO \citep{denyer2019}.
...
The CC-SPR pipeline, experiment scripts, and reproduction instructions
are available at
[[https://github.com/ChimdiWalter/Omics_Project_CCSPR]]. The pipeline
depends on GUDHI \citep{gudhi2014}, CVXPY, scikit-learn, and GSEApy
\citep{gseapy}.
...
including: mathematical background and simplicial homology definitions
