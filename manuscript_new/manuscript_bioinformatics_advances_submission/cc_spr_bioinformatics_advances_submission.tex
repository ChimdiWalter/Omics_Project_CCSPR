...
% ============================================================
%  ABSTRACT — Unstructured (Bioinformatics Advances style)
% ============================================================
\begin{abstract}
Persistent homology provides a multiscale topological summary of omics data,
but standard pipelines discard the geometric identity of detected cycles and
rely on fixed Euclidean metrics.
We introduce CC-SPR (Cycle-Consistent Sparse Persistent Representatives), which
addresses both limitations within the harmonic persistent homology scaffold.
First, we replace the dense $\ell_2$ representative with a convex elastic-net
objective that concentrates cycle support on a small number of edges, yielding
interpretable, bootstrap-stable feature-importance profiles.
Second, we formalize the sample-space metric as a selectable \emph{backend},
enabling systematic comparison of Euclidean, Ollivier--Ricci, diffusion,
PHATE-like, and distance-to-measure (DTM) geometries within a unified
filtration--representation--projection framework.
All five backends are benchmarked on three cancer-genomics datasets and a
plant developmental atlas.
Backend ranking is dataset-dependent and scale-sensitive: DTM leads on pilot
LUAD (F1${=}0.5315$); Ricci leads at full scale (F1${=}0.6339$); Euclidean leads
on GSE161711 (F1${=}0.7483$).
DTM uniquely recovers xenobiotic metabolism via cytochrome P450 on LUAD
(KEGG; adj.\ $p{=}7{\times}10^{-4}$), and zero genes are shared across all six
backend configurations.
CC-SPR produces 2.8$\times$ sparser sample-bar matrices than harmonic
representatives while preserving comparable clustering structure.
Source code: \url{https://github.com/ChimdiWalter/Omics_Project_CCSPR}.
\end{abstract}
...
This geometric richness makes omics a natural setting for topological data analysis (TDA) \citep{carlsson2009, edelsbrunner2010}, which has been applied to gene expression landscapes \citep{rizvi2017}, protein structure \citep{xia2014}, tumor microenvironments \citep{vipond2021}, single-cell trajectories \citep{moon2019phate}, neural population coding \citep{giusti2015}, and viral evolution \citep{chan2013topology}.
...
Harmonic persistent homology \citep{harmonicPH} addresses this gap by computing a representative cycle for each persistent class that minimizes an $\ell_2$ (harmonic) energy, creating a bridge from topological features back to individual samples and molecular features.
...
\item \textbf{Arabidopsis root atlas} (GSE123013, \citealt{denyer2019}, scRNA-seq, $n = 500$, 16 Leiden clusters): targeted cross-domain generalization test with Euclidean and DTM only.
\end{itemize}
...
The Vietoris--Rips filtration on $D^{(\mathcal{G})}$ produces a nested sequence of simplicial complexes whose homology yields a persistence barcode $\{[b_i, d_i)\}$ \citep{zomorodian2005, edelsbrunner2010}.
...
\end{equation}
This produces the smoothest cycle, typically with dense edge support ($\|z\|_0 = O(|E|)$), which dilutes biological signal when projected to feature space \citep{harmonicPH, friedman1998}.
...
The structure mirrors the elastic net \citep{zou2005elasticnet}: $\lambda_1$ plays the LASSO role promoting sparsity, $\lambda_2$ provides ridge-like stability, and the ``design matrix'' is the boundary operator $\partial_2$.
...
Backend choice can be understood as a controlled perturbation: when the distance matrices from two backends differ by $\varepsilon_\infty$ in the $\ell_\infty$ norm, the bottleneck distance between persistence diagrams is bounded by $\varepsilon_\infty$ \citep{chazal2014persistence}.
...
The Ollivier--Ricci curvature \citep{ollivier2007,ollivier2009} of an edge $(u,v)$ is $\kappa(u,v) = 1 - W_1(\mu_u, \mu_v)/d(u,v)$, where $W_1$ is the Wasserstein-1 distance between neighborhood measures.
Discrete Ricci flow \citep{ni2019community} iteratively updates edge weights: $w_{uv}^{(t+1)} = w_{uv}^{(t)} - \eta \cdot \kappa^{(t)}(u,v) \cdot w_{uv}^{(t)}$, upweighting negatively curved (bridge) edges.
...
\paragraph{Diffusion.}
Diffusion distances \citep{coifman2006diffusion} are derived from the eigendecomposition of a Markov transition matrix $P = D_W^{-1}W$, capturing connectivity that persists across random walks of varying length.
...
PHATE \citep{moon2019phate} transforms diffusion operators via a log potential: $U_i = -\log(P^t_{i,\cdot} + \epsilon)$, with potential distance $D^{(\text{phate})}(i,j) = \|U_i - U_j\|_2$.
...
DTM \citep{chazal2011dtm, chazal2017robust} replaces pairwise distances with density-smoothed variants: $d_{\mu_X, m_0}(x) = (\frac{1}{\lceil m_0 n \rceil} \sum_{j=1}^{\lceil m_0 n \rceil} \|x - x_{(j)}\|^2)^{1/2}$.
...
The dominant costs are distance matrix computation ($O(n^2 p)$ for Euclidean; higher for Ricci flow and diffusion eigendecomposition), persistent homology ($O(m^3)$ worst case, much faster in practice; \citealt{gudhi2014}), and sparse optimization ($O(|E|^2 \cdot |F|)$ per bootstrap).
...
Expanding the backend family and increasing sample size both improve CC-SPR: the original Ricci-only result (F1 $= 0.3133$) improves to DTM $= 0.5315$ at pilot scale and Ricci $= 0.6339$ at full scale.
...
\bottomrule
\end{tabular}
\caption{LUAD topological diagnostics. Euclidean produces the highest $H_1$ lifetime ($0.0798$) and most concentrated TIP (Gini $= 0.3212$). PHATE-like produces zero Gini, suggesting over-smoothing.}
\label{tab:diag_luad}
\end{table}
...
Key findings:
Euclidean's highest $H_1$ lifetime and TIP Gini on LUAD do not translate to the highest classification F1.
On GSE161711, Ricci creates the most persistent topological features ($H_1$ lifetime $= 0.1248$) but trails Euclidean on classification, illustrating that persistent topology captures structure complementary to class separation.
...
\caption{GSE161711 topological diagnostics. \textbf{Left}: TIP comparison. \textbf{Right}: $H_1$ lifetime comparison.}
\label{fig:tip_bulk}
\end{figure}

\textbf{Takeaway:} Topological diagnostics vary systematically across backends, providing information unavailable from classification metrics alone.

% ── 3.5 Pathway enrichment ───────────────────────────────────
\subsection{Pathway enrichment reveals backend-specific biology}
\label{sec:enrichment}

Over-representation analysis (ORA) using gseapy \citep{gseapy} against MSigDB, KEGG, and Reactome on the top 20 TIP genes per backend from LUAD reveals that each backend recovers different biology (Tables~\ref{tab:enrichment} and \ref{tab:geneoverlap}).
...
\emph{No genes are shared across all six backend configurations} (Table~\ref{tab:geneoverlap}), directly supporting the thesis that geometry choice controls which biological signal is extracted.
...
\textbf{Takeaway:} Different backends recover different, biologically coherent pathways.
...
Euclidean uniquely recovers NKX2-1 (TTF-1), a canonical LUAD lineage marker---not detected at pilot scale.
Ricci recovers innate immunity (DEFB4A) and squamous differentiation (SPRR3) markers.
Diffusion and PHATE-like both highlight cancer-testis antigens (MAGEA family).
...
\bottomrule
\end{tabular}
\caption{Top 5 TIP genes per backend on full-scale TCGA-LUAD ($n = 275$, 5-fold CV). Each backend selects distinct biology. NKX2-1 (rank 7, Euclidean) is a canonical LUAD lineage marker.}
...
Following \citet{harmonicPH}, we construct sample-level cycle-weight heatmaps for the top 15 $H_1$ bars (120 balanced LUAD samples, Euclidean backend).
CC-SPR produces only 18\% nonzero entries (vs 50\% for harmonic, 2.8$\times$ denser).
Ward hierarchical clustering recovers three sample clusters with comparable purity (0.43 for CC-SPR vs 0.47 for harmonic), demonstrating that sparsification preserves clustering structure.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/gurnari_style_comparison.png}
\caption{Sample-level cycle-weight heatmaps for LUAD (120 samples $\times$ 15 $H_1$ bars). \textbf{Top}: CC-SPR---18\% nonzero. \textbf{Bottom}: Harmonic---50\% nonzero.}
\label{fig:gurnari}
\end{figure}

\textbf{Takeaway:} CC-SPR achieves 2.8$\times$ sparser representations while preserving comparable clustering structure.

% ── 3.8 Lambda sensitivity ───────────────────────────────────
\subsection{Lambda sensitivity across backends}
\label{sec:lambda}

The sparsity parameter $\lambda$ shows contrasting behavior across datasets (Tables~\ref{tab:lambda_luad} and \ref{tab:lambda_gse}).
On LUAD, $\lambda$ has no effect; on GSE161711, most backends lose F1 at higher $\lambda$, but DTM uniquely \emph{improves}, suggesting its density-smoothed distances produce representatives where additional sparsity removes noise rather than signal.
...
\bottomrule
\end{tabular}
\caption{LUAD lambda ablation: no effect at either value.}
...
\bottomrule
\end{tabular}
\caption{GSE161711 lambda ablation: DTM uniquely improves with higher $\lambda$.}
...
Full ablation across six hyperparameter axes (Table~\ref{tab:ablationbest}) confirms cross-dataset divergence: LUAD benefits from Ricci geometry and compact top-$K$; GSE161711 benefits from Euclidean geometry and broad top-$K$.
...
\bottomrule
\end{tabular}
\caption{Best ablation settings. Cross-dataset divergence supports backend-sensitive behavior.}
...
\caption{Cross-dataset ablation profiles showing dataset-specific sensitivity to every hyperparameter axis.}
...
Three larger scRNA configurations failed under cluster limits (Table~\ref{tab:scRNAresource}); the bounded configuration ($n = 320$, 1 split, 1 bootstrap) completed with all five backends producing identical F1 $= 0.9254$, establishing pipeline feasibility but not a meaningful backend comparison.
...
DTM outperforms Euclidean on Arabidopsis root scRNA-seq (F1 $= 0.67$ vs $0.59$; 500 cells, 16 Leiden clusters, 3-fold CV), consistent with its LUAD advantage.
Euclidean selects transcription factors (FLA12, AGL1, IAA29), while DTM emphasizes cell wall remodeling (GATA2, CSLA9, EXPB3).
Three genes (IAA29, AT5G44005, PAR2) appear in both backends, suggesting auxin signaling is geometry-robust.
...
\bottomrule
\end{tabular}
\caption{Top 5 TIP genes per backend on Arabidopsis root.}
...
\bottomrule
\end{tabular}
\caption{Arabidopsis root topological diagnostics.}
\label{tab:diag_arab}
\end{table}

\textbf{Takeaway:} DTM's advantage generalizes across biological domains with coherent gene selections.
...
To test whether TIP scores converge at higher resample counts and whether CC-SPR's classification signal is above chance, we ran a dedicated stability and permutation study.

\paragraph{GSE161711 at $n_{\text{boot}} = 1000$.}
On 5{,}000 variable genes, all five backends completed $\geq 986/1000$ iterations ($> 98.6\%$).
The rank order of support sizes and Gini values is preserved between $n_{\text{boot}} = 5$ and $n_{\text{boot}} = 1000$ (Table~\ref{tab:gse_boot1000}), confirming that the original diagnostics captured the correct qualitative geometry ordering.
Total runtime across the five backends was approximately 8.3 hours.
...
\bottomrule
\end{tabular}
\caption{GSE161711 TIP stability at $n_{\text{boot}} = 1000$, 5{,}000 variable genes. Concentration ranking is preserved from the $n_{\text{boot}} = 5$ diagnostics.}
...
(ii)~at $n = 1{,}000$ cells (memory $\sim 14$~GB), per-iteration cost reached $\sim 12{,}000$ s due to ill-conditioned CVXPY solver subproblems on dense distance matrices.
At 500 cells and $n_{\text{boot}} = 10$, both Euclidean and DTM completed 10/10 iterations in $\sim 6$ h total (euclidean: support 46, Gini 0.323; dtm: support 48, Gini 0.289).
This is a fundamental limitation of the Rips + elastic-net approach, not an implementation artifact, and motivates future work on approximate persistent homology (sparse Rips, witness complexes) and better-conditioned cycle optimizers for large single-cell atlases.
...
First, sparse elastic representatives concentrate cycle support on compact gene sets that are more amenable to pathway analysis.
The 2.8$\times$ sparsification preserves clustering structure while dramatically improving interpretability.
Second, the backend-aware framework reveals that geometry choice materially controls which biological signal is extracted---zero genes are shared across all six configurations on LUAD.
Third, TIP profiles provide a principled stability diagnostic, enabling practitioners to assess which features are robustly associated with topological structure.

Even when CC-SPR trails the standard baseline on F1, the sparse representatives and TIP profiles identify gene--topology associations and geometry sensitivities that standard classifiers cannot detect.

% ── 4.2 When each backend helps ──────────────────────────────
\subsection{When each backend helps}

\textbf{Euclidean} performs best when classification-relevant structure aligns with ambient-space proximity (GSE161711).
\textbf{Ricci} helps when curvature correction suppresses misleading shortcut edges between subtypes (full-scale LUAD); its advantage strengthens with sample size.
\textbf{DTM} excels on heterogeneous data with density irregularity (pilot LUAD) and produces the most compact support (GSE161711, 24 features).
...
The DTM backend's xenobiotic metabolism enrichment (adj.\ $p < 0.001$) on LUAD has clear biological grounding: ALDH3A1, AKR1C1, AKR1C2, CYP4F11, and UGT1A6 are Phase~I/II detoxification enzymes known to vary across LUAD subtypes \citep{rizvi2017}, particularly the proximal-inflammatory subtype.

The zero-overlap finding is evidence that different geometric perspectives highlight different biological programs.
Practitioners can compare TIP profiles to identify geometry-robust genes (core signal) versus geometry-specific genes.

Cross-domain consistency strengthens these findings: DTM outperforms Euclidean on both LUAD and Arabidopsis, with biologically coherent gene selections in both domains.

% ── 4.4 Related methods ──────────────────────────────────────
\subsection{Related methods}

Most TDA applications in biology use standard Euclidean persistent homology without representative cycle analysis \citep{rizvi2017, xia2014, vipond2021, giusti2015, chan2013topology}.
Harmonic PH \citep{harmonicPH} introduced the bridge from persistent classes to sample-level representatives; CC-SPR preserves this scaffold while modifying the objective and generalizing the metric.
Ollivier--Ricci curvature \citep{ollivier2007, ollivier2009, ni2019community} has been used for community detection, but its connection to persistent homology through filtration perturbation appears novel.
Diffusion maps \citep{coifman2006diffusion} and PHATE \citep{moon2019phate} are widely used for dimensionality reduction; using these as persistent-homology backends within a sparse representative framework has not been systematized.
DTM-based topology \citep{chazal2011dtm, chazal2017robust} provides robust persistent homology, but its integration into a representative-cycle framework with feature projection appears unexplored.
Sparse representations in TDA have been studied through $\ell_1$ optimal cycles \citep{chen2011hardness, dey2011optimal}, but in computational-geometry settings without the biological feature-projection layer.
...
Pilot LUAD ranking (DTM $>$ Ricci) reverses at full scale.
\item \textbf{Evaluation protocol asymmetry.}
The standard baseline selects variable genes from the full dataset at full LUAD scale.
\item \textbf{PHATE-like uses a lightweight surrogate.}
Results may differ with the complete PHATE pipeline \citep{moon2019phate}.
...
Natural extensions include: integrating topology-derived features as auxiliary inputs to conventional classifiers; survival stratification using TIP-derived features; larger bootstrap studies for TIP convergence; the full PHATE pipeline; memory-optimized single-cell implementations; and extension to higher homology dimensions.
...
Zero genes are shared across all six configurations on LUAD.
...
CC-SPR produces 2.8$\times$ sparser sample-bar matrices while preserving comparable clustering structure.
...
CC-SPR is a complementary analysis tool.
\end{enumerate}
...
Arabidopsis root atlas (GSE123013) from GEO \citep{denyer2019}.

\section*{Code Availability}
The CC-SPR pipeline, experiment scripts, and reproduction instructions are available at \url{https://github.com/ChimdiWalter/Omics_Project_CCSPR}.
The pipeline depends on GUDHI \citep{gudhi2014}, CVXPY, scikit-learn, and GSEApy \citep{gseapy}.

\section*{Supplementary Information}
Supplementary data are available at \textit{Bioinformatics Advances} online, including:
...
% ============================================================
%  BIBLIOGRAPHY — Author-year with natbib annotations
% ============================================================
\bibliographystyle{plainnat}
\begin{thebibliography}{25}

\bibitem[Gurnari et~al.(2025)]{harmonicPH}
...
\bibitem[Vipond et~al.(2021)]{vipond2021}
...
\bibitem[Chazal et~al.(2011)]{chazal2011dtm}
...
\bibitem[Fang et~al.(2023)]{gseapy}
